//
// Created by Sarah Orsak on 11/11/2019.
//
/*  BST312.h
  CS 312 Fall 2018
...
void BST_312 <ItemType>::insertItem(TreeNode*& t, const ItemType& newItem)
{
    ...
    //if not, then move down tree and call recursively
    else if (newItem < t -> data){
...
{
    vector<ItemType> preTraversal;
    if (!isEmpty()){
        preOrderTraversal(root, preTraversal);
    }
    return preTraversal;
...
{
    vector<ItemType> postTraversal;
    if (!isEmpty()){
        postOrderTraversal(root, postTraversal);
    }
